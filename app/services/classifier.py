...
LAW_PATTERN = re.compile(
    # "стать\w*" вместо буквального "статья" — покрывает падежные формы
    # (статье, статьи, статьёй), которые в естественной русской речи
    # встречаются чаще, чем именительный падеж. Раньше регулярка ловила
    # только "ст." или ровно "статья" — конструкции вроде "согласно
    # статье 15" (частая формулировка в реальных официальных ответах)
    # не распознавались вовсе, и обоснованный ответ мог ошибочно
    # получить более низкую оценку, чем заслуживает.
    r"(?:ст\.|стать\w*|фз\s*№|федеральн\w*\s+закон|нк\s+рф|жк\s+рф|тк\s+рф|кас\s+рф)\s*[\d№]",
    re.IGNORECASE,
)
